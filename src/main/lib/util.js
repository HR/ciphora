...
function waitUntil(conditionFn, timeout, pollInterval = 30) {
  ...
  return new Promise((resolve, reject) => {
    (function wait() {
      ...
      else if (timeout && (Date.now() - start) >= timeout) return reject(new Error(`Timeout ${timeout} for waitUntil exceeded`))
...
function chunk(buffer, chunkSize) {
  ...
  let result = [], i = 0, len = buffer.length
